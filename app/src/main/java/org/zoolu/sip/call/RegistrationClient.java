...
public class RegistrationClient implements TransactionClientListener, TimerListener {
   ...
   private static final String TAG = "Sip: RegistrationClient";
   ...
   {  Log.v(TAG, "register with "+registrar+" for "+expire_time+" secs");
      attempts=0;
      ...
      if (body!=null)
      {  Log.v(TAG, "register body: "+body.length()+" bytes");
         req.setBody(body);
      }
      if (expire_time>0) Log.v(TAG, "registering contact "+contact+" (it expires in "+expire_time+" secs)");
      else Log.v(TAG, "unregistering contact "+contact);
   ...
   {  attempts=0;
      ...
      Log.v(TAG, "unregistering all contacts");
      TransactionClient t=new TransactionClient(sip_provider,req,this); 
      t.request(); 
   }
   ...
   {  if (transaction.getTransactionMethod().equals(SipMethods.REGISTER))
      {  if (resp.hasAuthenticationInfoHeader())
         ...
         {  attempt_to=null;
            ...
            Log.v(TAG, "next registration after "+renew_time+" secs");
         }
   ...
   {  if (transaction.getTransactionMethod().equals(SipMethods.REGISTER))
      {  StatusLine status=resp.getStatusLine();
         ...
         {  // UAS authentication
            ...
            //Log.v(TAG, "DEBUG: qop-options: "+qop_options,Log.LEVEL_MEDIUM);
         ...
         {  // Registration failure
            ...
            {  registration_to=null;
               ...
               Log.v(TAG, "next attempt after "+(SipStack.regc_max_attempt_timeout/1000)+" secs");
            }
   ...
   {  if (transaction.getTransactionMethod().equals(SipMethods.REGISTER))
      {  Log.v(TAG, "Registration failure: No response from server");
         if (loop)
         {  registration_to=null;
            ...
            Log.v(TAG, "next attempt after "+(inter_time_msecs/1000)+" secs");
         }
