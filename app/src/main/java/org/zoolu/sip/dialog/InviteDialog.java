    ...
    public void onReceivedMessage(SipProvider sip_provider, Message msg)
    {
        ...
        {  // check CSeq
            ...
            if (msg.isInvite())
            {
                ...
                // if ack (for 2xx)
                if (msg.isAck())
                {  if (!(statusIs(D_ACCEPTED)||statusIs(D_ReACCEPTED))) return;
                    changeStatus(D_CALL);
                    ...
                    // if bye
                    if (msg.isBye())
                    {  if (!(statusIs(D_CALL)||statusIs(D_BYEING))) return;
                        changeStatus(D_BYED);
                        ...
                        // if cancel
                        if (msg.isCancel())
                        {  if (!(statusIs(D_INVITED)||statusIs(D_ReINVITED))) return;
