...
public abstract class Dialog extends DialogInfo implements SipProviderListener {
    ...
    public static final String TAG = "Sip: Dialog";
  ...
  /** SipProvider */
   ...
   {  status=newstatus;
      Log.v(TAG, "changed dialog state: "+getStatus());
   ...
   {  
      if (isTerminated())
      {  Log.v(TAG, "trying to update a terminated dialog: do nothing.");
         return;
      }
      // else
      ...
      {  Log.v(TAG, "new dialog-id: "+new_id);
