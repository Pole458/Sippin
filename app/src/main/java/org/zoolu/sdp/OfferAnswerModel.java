...
public class OfferAnswerModel
{
   ...
   {  Vector new_media=new Vector();
    ...
    public static Vector<MediaDescriptor> makeMediaDescriptorProduct(Vector<MediaDescriptor> start_md_list, Vector<MediaDescriptor> offer_md_list) {

        Vector<MediaDescriptor> answer_md_list = new Vector<>();
        Vector<MediaDescriptor> aux_md_list = new Vector<>(start_md_list);

        for (int i=0; i<offer_md_list.size(); i++) {
            MediaDescriptor offer_md = offer_md_list.elementAt(i);
            String media_type=offer_md.getMedia().getMedia();
            for (int j=0; j<aux_md_list.size(); j++) {
                MediaDescriptor aux_md = aux_md_list.elementAt(j);
